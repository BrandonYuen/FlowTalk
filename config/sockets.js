...
module.exports.sockets = {
	...
	***************************************************************************/
	afterDisconnect: function(session, socket, cb) {
		// If a VALID session disconnects (prevents 'undefined' users from disconnecting after sails restart)
		if (session.username != undefined) {
			if (session.currentRoom == 'public') {
				session.currentRoom = '';
				sails.log.debug("User '"+session.username+session.userId+"' left the chat.");
				sails.sockets.broadcast('public', 'clientDisconnect', { username: session.username, userId: session.userId });
				return cb();
			}
		}
	},
